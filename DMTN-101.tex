...
using a median or 5-sigma-clip to make a "master" frame; the combination may be done using standard LSST image
stacking code.  The pocket-pumped data should not be included in the master frames, but should be median-
combined separately.
...
Tests:
\begin{enumerate}
\item Divide each resulting master frame by its median.
\item For each master frame, confirm that the median level is 1.0 to within statistical noise
(after masking known defects)
\item For each master frame, divide by the standard deviation image, and confirm that the 5-sigma
clipped standard is 1.0 to within statistical noise (after masking known defects)
\item for each pixel, check if all master frames are consistent to within statistical noise;
...
standard LSST image stacking code.  The resulting master bias will then be ingested.
...
Tests:
\begin{enumerate}
\item Fit the amplitude of each cross-talk image to the image of the "aggressor" spot, resulting in an 3024x3024
...
\item Large scale features such as the ``tree-rings'' due to variation in the properties of the
	silicon, or perturbations in the electric field (\eg ``edge distrotions`` [also known as ``glowing edges'' in certain devices]).
...
The Photon Transfer Curve (PTC) of a CCD plots the variance of an image taken under uniform illumination (``flat fields``) as a function of their average. It allows for the calculation of fundamental CCD parameters such as the gain (electrons per ADU), the reasout noise, and the mean pixel full well via the PTC turnoff. As such, it plays an important role in the characterization and monitoring of the state of a detector. 

It has been stablished that the variance flattens out at high fluxes and it is not perfectly proportional to the average flux, as Poissonian statistics would dictate. This is related to the ``Brighter-fatter`` effect and increases the correlations between adjacent pixels, decaying with distance, as reported in e.g., Astier et al. 2019. This study proposes a couple of models to fit to the PTC data (including covariances) with a leading order parameter to take the BFE into account. This parameter should be negative (as it describes the change in a pixel area due to its own charge content), is purely electrostatic in nature, and should not change during the life of a given CCD unless operating volategs such as the bias voltage are changed.  
...
\item Check that the brither-fatter effect parameter from the Astier et al. 2019 models, $a00$, is within a range determined by measurements of the distribution of this parameter (e.g., within five sigma of the mean of the coefficientis distribution for a given instrument, detector, and fit type).  
\end{enumerate}
